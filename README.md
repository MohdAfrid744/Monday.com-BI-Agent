...
Below is a breakdown of every file that remains in this submission and what each one does:
...
| `.gitignore` | Excludes temporary files, virtual environments, caches, and any non-submitted artifacts. |
| `.env.example` | Template showing what keys you need to provide. Copy this to `.env` and fill in your actual API credentials. |
| `.env` | (Not checked in) Your personal API keys; create by copying `.env.example` and filling in the values. |
...
1. **Clone the repository**
   ```bash
   git clone <your-repo-url> "AI Project - Copy"
   cd "AI Project - Copy"
   ```
...
4. **Configure environment variables**  
   The `.env.example` file shows you what keys are needed. To set up your own:
   ```bash
   # Copy the template
   cp .env.example .env
   ```
...
- No sample data files, no unit tests, and no auxiliary documentation remain in this submission version. The project is intentionally stripped to focus on the working agent core.
