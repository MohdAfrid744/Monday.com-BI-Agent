...
4. ONE TOOL AT A TIME: Make one tool call, wait for the result, then decide the next step. Never call tools inside other tool calls.
...
def chat_stream_with_tracing(prompt, conversation_history, trace_callback, tone="Straight Forward"):
    ...
    # Initialize - try Groq first, fall back to Gemini on rate limit
    client = None
    use_gemini = False
    ...
    if not client:
        raise ValueError("No API key available - please set GROQ_API_KEY")
    ...
    for loop in range(1, 11):
        ...
        if not tool_calls:
            ...
            prompt_follow = (
                ...
                "Do not include any bullet points or numbering - just plain questions."
            )
            try:
                ...
                # simple split into list - split on newlines and clean up
                followups = []
                for line in follow_text.splitlines():
                    line = line.strip()
                ...
                # Ensure we have at least some followups
                if not followups and follow_text:
                    # If parsing failed, just use the whole text as one followup
                    followups = [follow_text.strip()] if follow_text.strip() else []
            except Exception as e:
                # If follow-up generation fails, return empty list
                followups = []
